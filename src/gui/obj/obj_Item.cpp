...
namespace obj {
    ...
    Item* Item::cast(QTreeWidgetItem* aItem) {
        if (!aItem)
            return nullptr;
        return (aItem->whatsThis(kItemColumn) == "ObjectItem") ? static_cast<Item*>(aItem) : nullptr;
    }

    const Item* Item::cast(const QTreeWidgetItem* aItem) {
        if (!aItem)
            return nullptr;
        return (aItem->whatsThis(kItemColumn) == "ObjectItem") ? static_cast<const Item*>(aItem) : nullptr;
    }

    Item::Item(const QTreeWidget& aTree, core::ObjectNode& aNode): QTreeWidgetItem(), mTree(aTree), mNode(aNode) {
