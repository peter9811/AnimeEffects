...
public:
    ...
    static bool blendImage(uint8* aResult, const uint8* aBack, const QRect& aRectRB, const uint8* aFront,
    ...
    static XCMemBlock encodePlanePackBits(
        const uint8* aSrc, size_t aSrcLength, int aWidth, int aHeight, int aSrcStride);
    ...
    static bool makeChanneledImage(PSDFormat::ChannelList& aDst, const PSDFormat::Header& aHeader,
        const XCMemBlock& aSrc, ColorFormat aSrcFormat, int aSrcWidth, int aSrcHeight);
    ...
    static XCMemBlock makeInterleavedImage(const PSDFormat::Header& aHeader, const PSDFormat::ChannelList& aChannels,
