...
namespace mesh {
        ...
        if (!mBeganSplit) {
            if (aCursor.emitsLeftPressedEvent()) {
                if (mFocuser.vtxFocus()) {
                    ...
                    mRelayStart.pos = util::CollDetect::getPosOnSegment(mRelayStart.edge->seg(), modelCursorPos);
                    mRelayEnd = mRelayStart.pos;
                    mBeganSplit = true;
    ...
    SplitMode::RelayPoint SplitMode::getIntersection(
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (edge == aIgnoreEdge)
                continue;
            if (edge->vtx(0) == aIgnoreVtx || edge->vtx(1) == aIgnoreVtx)
                continue;
    ...
    void SplitMode::updateRelayPoints(const core::CameraInfo& aCamera) {
        ...
        if (mRelayPoints[0].edge) {
            ...
            for (auto face : checkFaces) {
                RelayPoint res = getIntersection(aCamera, *face, cursorSeg, nullptr, startVtx);
