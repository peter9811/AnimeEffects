...
namespace mesh {
    ...
    void Renderer::renderMesh(const MeshAccessor& aMesh) {
        ...
        for (auto& face : aMesh.faces()) {
            ...
            const QPointF vpos[3] = {
                getScreenPointF(vtx[0]->vec()), getScreenPointF(vtx[1]->vec()), getScreenPointF(vtx[2]->vec())};
        ...
        for (auto& edge : aMesh.edges()) {
            const QPointF vpos[2] = {getScreenPointF(edge->vtx(0)->vec()), getScreenPointF(edge->vtx(1)->vec())};
    ...
    void Renderer::renderDangling(const QVector<QVector2D>& aDangling, const QVector2D& aCursor) {
        ...
        if (count >= 2) {
            ...
            const QPointF vtx[2] = {getScreenPointF(aDangling[0]), getScreenPointF(aCursor)};
    ...
    void Renderer::renderFocus(const Focuser& aFocuser) {
        ...
        if (aFocuser.edgeFocus() || aFocuser.selectingEdge()) {
            auto edge = aFocuser.edgeFocus() ? aFocuser.edgeFocus() : aFocuser.selectingEdge();
            const QPointF vpos[2] = {getScreenPointF(edge->vtx(0)->vec()), getScreenPointF(edge->vtx(1)->vec())};
            setEdgeBrush(true);
            mPainter.drawLine(vpos[0], vpos[1]);
        }
    ...
    void Renderer::setFaceBrush(int aState) const {
        auto color = aState == 0 ? QColor(100, 100, 255, 60)
            : aState == 1        ? QColor(230, 150, 255, 80)
