...
QVector2D Triangle2DPos::get(const Triangle2D& aTriangle) const {
    XC_ASSERT(isValid());
    const QVector2D first = aTriangle.v[mIndex] * mFirst + aTriangle.v[(mIndex + 1) % 3] * (1.0f - mFirst);
...
QVector2D Triangle2DPos::get(const std::array<QVector2D, 3>& aTriangle) const {
    XC_ASSERT(isValid());
    const QVector2D first = aTriangle[mIndex] * mFirst + aTriangle[(mIndex + 1) % 3] * (1.0f - mFirst);
