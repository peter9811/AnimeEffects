...
//-------------------------------------------------------------------------------------------------
void pushTreeRowRecursive(util::TreePos& aDst, const QTreeWidget& aTree, QTreeWidgetItem* aObj) {
    if (!aObj)
        return;
...
namespace res {
    ...
    Item* Item::cast(QTreeWidgetItem* aItem) {
        return (aItem->whatsThis(kItemColumn) == "ResourceItem") ? static_cast<Item*>(aItem) : nullptr;
    }

    const Item* Item::cast(const QTreeWidgetItem* aItem) {
        return (aItem->whatsThis(kItemColumn) == "ResourceItem") ? static_cast<const Item*>(aItem) : nullptr;
    }

    Item::Item(const QTreeWidget& aTree, img::ResourceNode& aNode, const QString& aIdentifier):
        QTreeWidgetItem(), mTree(aTree), mNode(aNode) {
