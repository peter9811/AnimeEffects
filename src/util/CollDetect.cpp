...
bool CollDetect::rayIntersectsToPolygonEdge(const QVector2D& a, const QVector2D& b, const QVector2D& p) {
...
bool CollDetect::intersects(const QRectF& aRect, const Segment2D& aSegment) {
    if (aRect.contains(aSegment.start.toPointF()))
        return true;
    if (aRect.contains(aSegment.end().toPointF()))
        return true;
    ...
    if (util::CollDetect::intersects(util::Segment2D(tl, tr - tl), aSegment))
        return true;
    if (util::CollDetect::intersects(util::Segment2D(tr, br - tr), aSegment))
        return true;
    if (util::CollDetect::intersects(util::Segment2D(br, bl - br), aSegment))
        return true;
    if (util::CollDetect::intersects(util::Segment2D(bl, tl - bl), aSegment))
        return true;

    return false;
}

std::pair<bool, QVector2D> CollDetect::getIntersection(const Segment2D& aSeg0, const Segment2D& aSeg1) {
    float cross = getCross(aSeg0.dir, aSeg1.dir);
    if (cross == 0.0f) {
