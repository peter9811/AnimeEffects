...
BufferObject::BufferObject(GLenum aType):
    mType(aType), mId(0), mTypeSize(0), mUsingDataCount(0), mBufferCount(0), mUsage() {
...
void BufferObject::resetRawData(GLsizeiptr aTypeSize, const GLvoid* aData, int aDataCount, GLenum aUsage) {
...
void BufferObject::copyFrom(const BufferObject& aFrom) {
    XC_MSG_ASSERT(mUsingDataCount == aFrom.dataCount(), "data count: %d, %d", mUsingDataCount, aFrom.dataCount());
