...
QPointF MathUtil::getRotateVectorRad(const QPointF& aVec, float aRotate) {
    const double s = qSin(aRotate);
    const double c = qCos(aRotate);
...
QVector2D MathUtil::blendVectorByClockwiseRotation(const QVector2D& aFrom, const QVector2D& aTo, float aRate) {
