...
void TreePos::updateByRemove(const TreePos& aRemovePos) {
    if (!isValid() || !aRemovePos.isValid())
        return;
    if (depth() < aRemovePos.depth())
        return;

    for (int i = 0; i < aRemovePos.depth() - 1; ++i) {
        if (row(i) != aRemovePos.row(i))
            return;
    }
...
void TreePos::updateByInsert(const TreePos& aInsertPos) {
    if (!isValid() || !aInsertPos.isValid())
        return;
    if (depth() < aInsertPos.depth())
        return;

    for (int i = 0; i < aInsertPos.depth() - 1; ++i) {
        if (row(i) != aInsertPos.row(i))
            return;
    }
...
void TreePos::dump() const {
    ...
    for (size_t i = 0; i < mRows.size(); ++i) {
        QString row = QString("%1,").arg(mRows[i]);
        text += row;
    }
    qDebug() << text;
}
