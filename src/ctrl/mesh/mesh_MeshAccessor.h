...
namespace mesh {
    ...
    public:
        ...
        void setKey(core::MeshKey& aKey) { mKey = &aKey; }

        cmnd::Vector
        createTrianglePusher(const QVector<QVector2D>& aPos, const QVector<MeshVtx*>& aRef, MeshFace** aDst) {
            return mKey->createTrianglePusher(aPos, aRef, aDst);
        }

        cmnd::Vector createRemover(MeshVtx& aVtx) { return mKey->createRemover(aVtx); }

        cmnd::Vector createRemover(MeshFace& aFace) { return mKey->createRemover(aFace); }

        cmnd::Vector
        createSplitter(MeshFace& aFace, MeshEdge& aEdgeSide, const QVector2D& aPosOnEdge, MeshVtx** aTail) {
